...
def auto_buy():
    ...
    while(amount_remaining > 0):
        ...
        if(random_storage.get_status() < random_storage.max_storage):
            validtypes = random_storage.stored_types

            # Filter out loot if it can be stored (player cannot buy loot)
            if ItemType.Loot in validtypes: validtypes.remove(ItemType.Loot)
...
def shop_phase():
    for i in range(daily_customers):
        if(i != 0):
            print("")
            input("Press enter to greet the next customer ")
            print("")
...
def approach_counter(customer: Customer, chosen_items, item_to_sell):
    ...
    if(len(chosen_items) != 0 and random_roll(customer.haggle_chance)):
        ...
        haggle_item_price = get_adjusted_price(haggle_item)
        # Adjust for margins
        # The amount of silver the customer wants off
        # minimum 10% of the price, max 25
        haggle_amount = randint(haggle_item_price // 10, haggle_item_price // 5)
        if(haggle_amount == 0): 
            haggle_amount = 1
        # Print the request
        print(f"{get_dialogue(DialogueType.haggle_attempt)}")
        sleep(0.3)
        print(f"{haggle_item}: {haggle_item_price} -> {haggle_item_price - haggle_amount} ({haggle_amount} silver discount)")
        sleep(0.3)
        if(yes_no_input("Accept?")):
            # The customer buys the item instantly with the new price
            if(random_roll(30)): change_reputation(1)
            change_player_currency(haggle_item_price - haggle_amount)
            stats["Items sold"] += 1
...
def get_stock_value() -> int:
    to_return = 0
    for storage in storages:
        for item, amount in storage.held_items.items():
            to_return += item.price * amount
    return to_return
...
def offer_item(item_to_sell):
    ...
    sleep(0.4)
    print(f"{item_to_sell} - {item_to_sell.price} silver (you have {silver})")
    sleep(0.4)
    if(yes_no_input("Accept?")):
...
def day_summary(day_start_silver, day_start_stock_value):

    # Change profit by the raw value of stock sold
    # Ex: gained 500 dollars but spent 250 on the stock so profit is 250
    profit = (silver - day_start_silver) + (get_stock_value() - day_start_stock_value)
    ...
    sleep(0.3)
    print(f"You have {get_stock_value()} silver worth of stock.")
    sleep(0.3)
...
def calculate_operating_cost() -> int:
    # Increases per day
    operating_cost = (default_operating_cost + (day * precent_operating_cost_step))
...
def win():
    clear_console()
    print(f"You collected {goal_silver} silver and won!")
    sleep(0.3)
    print("Your final stats:")
    print_stats()
    sleep(0.3)
...
def main_menu():
    ...
    print(f"Make {goal_silver} silver or go bankrupt trying!")
